...
require('./backend/migrate')(db);
function maybeBackupDatabase(){
  if(String(process.env.BRAVE_AUTO_DB_BACKUP||'').toLowerCase()!=='true') return;
  try{
    const dir=process.env.BRAVE_MIGRATED_DIR||path.join(__dirname,'storage','migrated','database');
...
function safeUser(u){if(!u)return null;return {id:u.brave_id,username:u.username,fullname:u.fullname,email:u.email,phone:u.phone||'',accountType:u.account_type||'Buyer',country:u.country,profileImage:u.profile_image||'',accountStatus:u.account_status||'active',verificationStatus:u.verification_status||'unverified',emailVerified:!!u.email_verified,phoneVerified:!!u.phone_verified,verificationSerial:u.verification_serial||'',location:u.location||u.country||'',rating:Number(u.rating||0),ratingCount:Number(u.rating_count||0),createdAt:u.created_at};}
function cookieValue(req,name){
 const raw=String(req.headers.cookie||'');
 const part=raw.split(';').map(x=>x.trim()).find(x=>x.startsWith(name+'='));
 return part ? decodeURIComponent(part.slice(name.length+1)) : '';
}
function currentUser(req){const control=clean(req.headers['x-admin-control-token']); if(control){try{const h=crypto.createHash('sha256').update(control).digest('hex');const c=db.prepare('SELECT target_user_id,expires_at FROM admin_control_sessions WHERE admin_token_hash=? AND status=\"active\"').get(h);if(c&&new Date(c.expires_at).getTime()>Date.now()){const u=db.prepare('SELECT * FROM users WHERE brave_id=?').get(c.target_user_id);if(u){u.__adminControl=true;return u;}}}catch(_){} } const t=clean((req.headers.authorization||'').replace(/^Bearer\s+/i,'')||req.headers['x-session-token']||cookieValue(req,'brave_session')); if(!t)return null; const mem=userSessions.get(t); if(mem){const u=db.prepare('SELECT * FROM users WHERE brave_id=?').get(mem.userId); if(u)return u;} try{const h=crypto.createHash('sha256').update(t).digest('hex'); const row=db.prepare('SELECT user_id,expires_at FROM user_sessions WHERE token_hash=?').get(h); if(!row||new Date(row.expires_at).getTime()<Date.now())return null; const u=db.prepare('SELECT * FROM users WHERE brave_id=?').get(row.user_id); if(u){userSessions.set(t,{userId:u.brave_id,createdAt:Date.now(),rememberMe:true}); db.prepare('UPDATE user_sessions SET last_seen=? WHERE token_hash=?').run(now(),h); return u;} }catch(_){} return null;}
function requireUser(req,res,next){
 const u=currentUser(req); if(!u)return res.status(401).json({message:'Please log in to continue.'});
 if(u.account_status!=='active' && !u.__adminControl)return res.status(403).json({message:'Your account is not currently active.'});
 try{
   const t=clean((req.headers.authorization||'').replace(/^Bearer\s+/i,'')||req.headers['x-session-token']||cookieValue(req,'brave_session'));
   if(t){const h=crypto.createHash('sha256').update(t).digest('hex');const s=db.prepare('SELECT remember_me,expires_at FROM user_sessions WHERE token_hash=?').get(h);
     if(s&&Number(s.remember_me)===1){const refreshDays=Math.max(1,Number(process.env.BRAVE_SESSION_DAYS||90));const remaining=new Date(s.expires_at).getTime()-Date.now();if(remaining<14*24*60*60*1000){const exp=new Date(Date.now()+refreshDays*24*60*60*1000).toISOString();db.prepare('UPDATE user_sessions SET expires_at=?,last_seen=? WHERE token_hash=?').run(exp,now(),h);const secureCookie=process.env.NODE_ENV==='production'||req.secure||req.get('x-forwarded-proto')==='https';res.setHeader('Set-Cookie',`brave_session=${encodeURIComponent(t)}; Max-Age=${refreshDays*24*60*60}; Path=/; HttpOnly; SameSite=Lax${secureCookie?'; Secure':''}`);}}
   }
 }catch(_){ }
 req.user=u;next();
}
...
function requireOwnerOrAdmin(req,res,next){const t=clean(req.headers['x-admin-token']);if(t && adminSessions.has(t)){req.admin=true;return next();}const u=currentUser(req);if(u && u.account_status==='active'){req.user=u;req.admin=false;return next();}return res.status(401).json({message:'Please log in to manage this listing.'});}
...
app.get('/api/session/bootstrap',(req,res)=>{
  const existing=clean((req.headers.authorization||'').replace(/^Bearer\s+/i,'')||req.headers['x-session-token']||cookieValue(req,'brave_session'));
  const u=currentUser(req);
  if(!u)return res.status(401).json({message:'Your BRAVE session has expired. Please log in again.'});
  const token=crypto.randomBytes(32).toString('hex');
  const remember=true;
  const sessionDays=Math.max(1,Number(process.env.BRAVE_SESSION_DAYS||90));
  const expires=new Date(Date.now()+sessionDays*24*60*60*1000).toISOString();
  try{
    if(existing){db.prepare('DELETE FROM user_sessions WHERE token_hash=?').run(crypto.createHash('sha256').update(existing).digest('hex'));userSessions.delete(existing);}
    const hash=crypto.createHash('sha256').update(token).digest('hex');
    db.prepare('INSERT INTO user_sessions(token_hash,user_id,remember_me,expires_at,ip,user_agent) VALUES(?,?,?,?,?,?)').run(hash,u.brave_id,1,expires,req.ip,clean(req.get('user-agent')));
    userSessions.set(token,{userId:u.brave_id,createdAt:Date.now(),rememberMe:true});
    const secureCookie=process.env.NODE_ENV==='production'||req.secure||req.get('x-forwarded-proto')==='https';
    res.setHeader('Set-Cookie',`brave_session=${encodeURIComponent(token)}; Max-Age=${sessionDays*24*60*60}; Path=/; HttpOnly; SameSite=Lax${secureCookie?'; Secure':''}`);
    try{storageService.save(db,u.brave_id);}catch(e){console.error('[STORAGE] bootstrap snapshot warning:',e.message);}
    return res.json({user:safeUser(u),token,rememberMe:true});
  }catch(e){console.error('[SESSION] bootstrap failed:',e.message);return res.status(500).json({message:'Unable to restore your BRAVE session right now.'});}
});
...
app.post('/api/account/profile',requireUser,(req,res)=>{const name=clean(req.body.fullname);if(!name)return res.status(400).json({message:'Name is required.'});const location=clean(req.body.location).slice(0,160);db.prepare('UPDATE users SET fullname=?,location=?,updated_at=? WHERE brave_id=?').run(name,location,now(),req.user.brave_id);res.json({message:'Profile updated.',user:safeUser(db.prepare('SELECT * FROM users WHERE brave_id=?').get(req.user.brave_id))});});
...
app.post('/api/products',requireUser,(req,res)=>{const name=clean(req.body.name||req.body.title);if(!name)return res.status(400).json({message:'Product name is required.'});const image=base64(req.body.image),video=base64(req.body.video),pdf=base64(req.body.pdf),audio=base64(req.body.audio),file=base64(req.body.file);for(const [v,label,rx] of [[image,'image',/^data:image\//],[video,'video',/^data:video\//],[pdf,'PDF',/^data:application\/pdf(?:;|,)/],[audio,'audio',/^data:audio\//],[file,'file',/^data:[^;]+;base64,/]])if(v&&!rx.test(v))return res.status(400).json({message:`Invalid ${label} upload.`});const pid=id('product');db.prepare(`INSERT INTO products(public_id,owner_id,owner_name,owner_username,name,category,description,price,delivery_price,payment_method,image_data,video_data,pdf_data,audio_data,file_data,file_name,file_type,featured,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(pid,req.user.brave_id,req.user.fullname,req.user.username,name,clean(req.body.category)||'General',clean(req.body.description),req.body.price===''?null:Number(req.body.price)||null,req.body.deliveryPrice===''?null:Number(req.body.deliveryPrice)||null,normalizePayment(req.body.paymentMethod,'product'),image,video,pdf,audio,file,clean(req.body.fileName).slice(0,180),clean(req.body.fileType).slice(0,120),0,'active');db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),req.user.brave_id,'Product published','Your product is now visible in the marketplace.');db.prepare('INSERT INTO timeline_posts(public_id,user_id,username,fullname,text,media_data,media_type,status) VALUES(?,?,?,?,?,?,?,?)').run(id('post'),req.user.brave_id,req.user.username,req.user.fullname,`Listed a new product: ${name}`,'','text','active');res.status(201).json({message:'Product published successfully.',product:db.prepare('SELECT * FROM products WHERE public_id=?').get(pid)});});
app.post('/api/admin/products',requireAdmin,(req,res)=>{const name=clean(req.body.name);if(!name)return res.status(400).json({message:'Product name is required.'});const pid=id('product');db.prepare(`INSERT INTO products(public_id,owner_id,owner_name,owner_username,name,category,description,price,delivery_price,payment_method,image_data,video_data,pdf_data,featured,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(pid,'ADMIN','UNIQUE BRAVE','',name,clean(req.body.category)||'Featured',clean(req.body.description),req.body.price===''?null:Number(req.body.price)||null,req.body.deliveryPrice===''?null:Number(req.body.deliveryPrice)||null,normalizePayment(req.body.paymentMethod,'product'),base64(req.body.image),base64(req.body.video),base64(req.body.pdf),1,'active');audit('admin_product_posted','product',pid,name);res.status(201).json({message:'Admin product posted.',product:db.prepare('SELECT * FROM products WHERE public_id=?').get(pid)});});
...
app.post('/api/services',requireUser,(req,res)=>{const name=clean(req.body.name||req.body.title);if(!name)return res.status(400).json({message:'Service name is required.'});const image=base64(req.body.image),video=base64(req.body.video),pdf=base64(req.body.pdf),audio=base64(req.body.audio),file=base64(req.body.file);for(const [v,label,rx] of [[image,'image',/^data:image\//],[video,'video',/^data:video\//],[pdf,'PDF',/^data:application\/pdf(?:;|,)/],[audio,'audio',/^data:audio\//],[file,'file',/^data:[^;]+;base64,/]])if(v&&!rx.test(v))return res.status(400).json({message:`Invalid ${label} upload.`});const sid=id('service');db.prepare(`INSERT INTO services(public_id,owner_id,owner_name,owner_username,name,category,description,price,payment_method,image_data,video_data,pdf_data,audio_data,file_data,file_name,file_type,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(sid,req.user.brave_id,req.user.fullname,req.user.username,name,clean(req.body.category)||'Professional service',clean(req.body.description),req.body.price===''?null:Number(req.body.price)||null,normalizePayment(req.body.paymentMethod,'service'),image,video,pdf,audio,file,clean(req.body.fileName).slice(0,180),clean(req.body.fileType).slice(0,120),'active');db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),req.user.brave_id,'Service published','Your service is now visible in the marketplace.');db.prepare('INSERT INTO timeline_posts(public_id,user_id,username,fullname,text,media_data,media_type,status) VALUES(?,?,?,?,?,?,?,?)').run(id('post'),req.user.brave_id,req.user.username,req.user.fullname,`Added a new service: ${name}`,'','text','active');res.status(201).json({message:'Service published successfully.',service:db.prepare('SELECT * FROM services WHERE public_id=?').get(sid)});});
...
app.get('/api/chat-requests',requireUser,(req,res)=>{
 const rows=db.prepare(`SELECT cr.*,u.fullname requester_name,u.username requester_username,ru.fullname recipient_name,ru.username recipient_username
 FROM chat_requests cr LEFT JOIN users u ON u.brave_id=cr.requester_id LEFT JOIN users ru ON ru.brave_id=cr.recipient_id
 WHERE cr.requester_id=? OR cr.recipient_id=? ORDER BY cr.id DESC LIMIT 200`).all(req.user.brave_id,req.user.brave_id);
 res.json({requests:rows});
});
app.post('/api/chat-requests',requireUser,(req,res)=>{
 const type=clean(req.body.listingType), listingId=clean(req.body.listingId), message=clean(req.body.message);
 if(!['product','service'].includes(type)||!listingId||!message)return res.status(400).json({message:'Listing and message are required.'});
 const table=type==='product'?'products':'services';
 const listing=db.prepare(`SELECT * FROM ${table} WHERE public_id=? AND status='active'`).get(listingId);
 if(!listing)return res.status(404).json({message:'Listing is no longer available.'});
 let recipient=listing.owner_id;
 if(['ADMIN','CATALOGUE_STAGING','catalog','demo'].includes(recipient)) recipient='ADMIN';
 if(!recipient||recipient===req.user.brave_id)return res.status(400).json({message:'You cannot request a chat with yourself.'});
 const targetUser=recipient==='ADMIN'?null:db.prepare('SELECT * FROM users WHERE brave_id=? AND account_status=?').get(recipient,'active');
 if(recipient!=='ADMIN'&&!targetUser)return res.status(404).json({message:'The listing owner is not currently available for chat.'});
 const existing=db.prepare(`SELECT * FROM chat_requests WHERE requester_id=? AND recipient_id=? AND listing_type=? AND listing_id=? AND status='pending'`).get(req.user.brave_id,recipient,type,listingId);
 if(existing)return res.json({message:'Your message request is already waiting for acceptance.',request:existing});
 const rid=id('chatreq');
 db.prepare(`INSERT INTO chat_requests(public_id,requester_id,recipient_id,listing_type,listing_id,listing_name,initial_message,status) VALUES(?,?,?,?,?,?,?,?)`).run(rid,req.user.brave_id,recipient,type,listingId,listing.name,message,'pending');
 const title=recipient==='ADMIN'?'New marketplace message request':'New marketplace chat request';
 if(recipient!=='ADMIN')db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),recipient,title,`${req.user.fullname} wants to discuss your ${type}: ${listing.name}`);
 audit('chat_request_created','listing',listingId,`${req.user.brave_id} -> ${recipient}`);
 res.status(201).json({message:'Message request sent. The chat will open after it is accepted.',request:db.prepare('SELECT * FROM chat_requests WHERE public_id=?').get(rid)});
});
app.post('/api/chat-requests/:id/action',requireUser,(req,res)=>{
 const action=clean(req.body.action);
 const cr=db.prepare('SELECT * FROM chat_requests WHERE public_id=?').get(req.params.id);
 if(!cr)return res.status(404).json({message:'Message request not found.'});
 if(cr.recipient_id!==req.user.brave_id && cr.requester_id!==req.user.brave_id)return res.status(403).json({message:'This message request is not yours.'});
 ...
 if(cr.recipient_id!==req.user.brave_id)return res.status(403).json({message:'Only the recipient can accept this request.'});
 db.prepare("UPDATE chat_requests SET status='accepted',accepted_at=?,updated_at=? WHERE public_id=?").run(now(),now(),cr.public_id);
 db.prepare('INSERT INTO messages(public_id,sender_id,receiver_id,message,context,payment_notice) VALUES(?,?,?,?,?,?)').run(id('msg'),cr.requester_id,cr.recipient_id,cr.initial_message,`marketplace:${cr.listing_type}:${cr.listing_id}|${cr.listing_name}`,cr.listing_type==='product'?'Confirm price, stock, delivery and payment terms before paying.':'Discuss scope, timing, location and price before paying.');
 db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),cr.requester_id,'Chat request accepted',`${req.user.fullname} accepted your request about ${cr.listing_name}. You can now continue the chat.`);
 audit('chat_request_accepted','listing',cr.listing_id,cr.public_id);
 res.json({message:'Chat accepted. You can now continue the conversation.',request:db.prepare('SELECT * FROM chat_requests WHERE public_id=?').get(cr.public_id)});
});
...
app.post('/api/orders',requireUser,(req,res)=>{
 ...
 tx();
 db.prepare('INSERT INTO timeline_posts(public_id,user_id,username,fullname,text,media_data,media_type,status) VALUES(?,?,?,?,?,?,?,?)').run(id('post'),req.user.brave_id,req.user.username,req.user.fullname,`Marketplace activity: placed order ${purchaseSerial}`,'','text','active');
 for(const r of rows){ if(r.p.owner_id && r.p.owner_id!=='ADMIN' && r.p.owner_id!==req.user.brave_id){ const su=db.prepare('SELECT username,fullname FROM users WHERE brave_id=?').get(r.p.owner_id); if(su) db.prepare('INSERT INTO timeline_posts(public_id,user_id,username,fullname,text,media_data,media_type,status) VALUES(?,?,?,?,?,?,?,?)').run(id('post'),r.p.owner_id,su.username,su.fullname,`Marketplace activity: a customer ordered ${r.p.name}`,'','text','active'); }}
...
app.post('/api/admin/subscriptions/:id/action',requireAdmin,(req,res)=>{const a=clean(req.body.action),s=db.prepare('SELECT * FROM subscriptions WHERE public_id=?').get(req.params.id);if(!s)return res.status(404).json({message:'Subscription not found.'});if(a==='activate'){const ends=new Date(Date.now()+30*24*60*60*1000).toISOString();db.prepare("UPDATE subscriptions SET status='active',starts_at=?,ends_at=? WHERE public_id=?").run(now(),ends,s.public_id);db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),s.user_id,'Plan activated','Your paid BRAVE plan has been verified and activated.');audit('subscription_activated','subscription',s.public_id,s.plan_code);return res.json({message:'Plan activated.'})}if(a==='reject'){db.prepare("UPDATE subscriptions SET status='rejected',ends_at=NULL WHERE public_id=?").run(s.public_id);db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),s.user_id,'Plan purchase declined','Your plan payment request was not approved. Contact Customer Care if you need help.');audit('subscription_rejected','subscription',s.public_id,s.plan_code);return res.json({message:'Plan purchase declined.'})}return res.status(400).json({message:'Invalid subscription action.'});});
...
app.post('/api/admin/users/:id/control',requireAdmin,(req,res)=>{
 ...
 const cid=id('control'),expires=new Date(Date.now()+30*60*1000).toISOString();
 db.prepare('INSERT INTO admin_control_sessions(public_id,admin_token_hash,target_user_id,purpose,expires_at) VALUES(?,?,?,?,?)').run(cid,hash,u.brave_id,clean(req.body.purpose)||'account_review',expires);
 audit('admin_account_control_started','user',u.brave_id,'30 minute controlled session');
 res.json({message:'Controlled account session created for 30 minutes.',controlToken:raw,expiresAt:expires,user:safeUser(u)});
});
app.post('/api/admin/control/end',requireAdmin,(req,res)=>{const token=clean(req.body.controlToken);if(token){const h=crypto.createHash('sha256').update(token).digest('hex');db.prepare("UPDATE admin_control_sessions SET status='closed' WHERE admin_token_hash=?").run(h)}res.json({message:'Controlled account session closed.'});});
app.get('/api/admin/users/:id/overview',requireAdmin,(req,res)=>{const u=db.prepare('SELECT * FROM users WHERE brave_id=?').get(req.params.id);if(!u)return res.status(404).json({message:'User not found.'});res.json({user:safeUser(u),products:db.prepare('SELECT * FROM products WHERE owner_id=? ORDER BY id DESC').all(u.brave_id),services:db.prepare('SELECT * FROM services WHERE owner_id=? ORDER BY id DESC').all(u.brave_id),messages:db.prepare('SELECT * FROM messages WHERE sender_id=? OR receiver_id=? ORDER BY id DESC LIMIT 200').all(u.brave_id,u.brave_id),reports:db.prepare('SELECT * FROM reports WHERE reporter_id=? ORDER BY id DESC').all(u.brave_id)});});
...
require('./backend/brave_completion')({app,db,helpers:{requireUser,requireAdmin,clean,base64,id,now,serial15,audit,publicUrl,safeUser}});
require('./backend/platform_final')({app,db,helpers:{requireUser,requireAdmin,clean,base64,id,now,serial15,audit,publicUrl,safeUser}});
...
/* UNIQUE BRAVE persistent cart + live user inbox APIs */
app.get('/api/cart',requireUser,(req,res)=>{
 const rows=db.prepare(`SELECT c.*,p.name,p.price,p.delivery_price,p.stock,p.quantity AS product_quantity,p.status,p.image_data,p.image_url FROM cart_items c LEFT JOIN products p ON p.public_id=c.product_id WHERE c.user_id=? ORDER BY c.id DESC`).all(req.user.brave_id);
 res.json({items:rows});
});
app.post('/api/cart',requireUser,(req,res)=>{
 const productId=clean(req.body.productId), qty=Math.max(1,Math.min(99,Number(req.body.quantity)||1));
 const p=db.prepare("SELECT * FROM products WHERE public_id=? AND status='active'").get(productId);
 if(!p)return res.status(404).json({message:'Product is no longer available.'});
 const variant=typeof req.body.variant==='object'&&req.body.variant?JSON.stringify(req.body.variant):String(req.body.variant||'');
 const existing=db.prepare('SELECT * FROM cart_items WHERE user_id=? AND product_id=? AND variant_json=?').get(req.user.brave_id,productId,variant);
 if(existing){db.prepare('UPDATE cart_items SET quantity=?,updated_at=? WHERE id=?').run(Math.min(99,existing.quantity+qty),now(),existing.id);}
 else db.prepare('INSERT INTO cart_items(public_id,user_id,product_id,variant_json,quantity,updated_at) VALUES(?,?,?,?,?,?)').run(id('cart'),req.user.brave_id,productId,variant,qty,now());
 res.status(201).json({message:'Added to your BRAVE cart.',count:db.prepare('SELECT COALESCE(SUM(quantity),0) c FROM cart_items WHERE user_id=?').get(req.user.brave_id).c});
});
app.patch('/api/cart/:id',requireUser,(req,res)=>{const q=Math.max(0,Math.min(99,Number(req.body.quantity)||0));const row=db.prepare('SELECT * FROM cart_items WHERE public_id=? AND user_id=?').get(req.params.id,req.user.brave_id);if(!row)return res.status(404).json({message:'Cart item not found.'});if(q===0)db.prepare('DELETE FROM cart_items WHERE public_id=? AND user_id=?').run(row.public_id,req.user.brave_id);else db.prepare('UPDATE cart_items SET quantity=?,updated_at=? WHERE public_id=? AND user_id=?').run(q,now(),row.public_id,req.user.brave_id);res.json({message:'Cart updated.'});});
app.delete('/api/cart/:id',requireUser,(req,res)=>{db.prepare('DELETE FROM cart_items WHERE public_id=? AND user_id=?').run(req.params.id,req.user.brave_id);res.json({message:'Removed from cart.'});});
app.delete('/api/cart',requireUser,(req,res)=>{db.prepare('DELETE FROM cart_items WHERE user_id=?').run(req.user.brave_id);res.json({message:'Cart cleared.'});});
app.get('/api/chat-requests/unread-count',requireUser,(req,res)=>res.json({count:db.prepare("SELECT COUNT(*) c FROM chat_requests WHERE recipient_id=? AND status='pending'").get(req.user.brave_id).c}));

/* UNIQUE BRAVE 2.6 business-platform upgrade: additive APIs */
app.get('/api/saved-listings',requireUser,(req,res)=>{
  res.json({items:db.prepare(`SELECT * FROM saved_listings WHERE user_id=? ORDER BY id DESC`).all(req.user.brave_id)});
});
app.post('/api/saved-listings',requireUser,(req,res)=>{
  const type=['product','service'].includes(clean(req.body.listingType))?clean(req.body.listingType):'product';
  const listingId=clean(req.body.listingId);
  if(!listingId)return res.status(400).json({message:'Listing ID is required.'});
  const table=type==='product'?'products':'services';
  const found=db.prepare(`SELECT public_id,owner_id FROM ${table} WHERE public_id=? AND status='active'`).get(listingId);
  if(!found)return res.status(404).json({message:'Listing not found.'});
  const sid=id('save');
  try{db.prepare('INSERT INTO saved_listings(public_id,user_id,listing_type,listing_id) VALUES(?,?,?,?,?)'.replace('?,?,?,?,?','?,?,?,?')).run(sid,req.user.brave_id,type,listingId);}
  catch(e){ if(!String(e.message).includes('UNIQUE')) throw e; }
  res.status(201).json({message:'Saved to your BRAVE list.'});
});
app.delete('/api/saved-listings/:type/:id',requireUser,(req,res)=>{
  db.prepare('DELETE FROM saved_listings WHERE user_id=? AND listing_type=? AND listing_id=?').run(req.user.brave_id,clean(req.params.type),clean(req.params.id));
  res.json({message:'Removed from saved listings.'});
});

app.get('/api/creative-designs',requireUser,(req,res)=>{
  res.json({designs:db.prepare('SELECT * FROM creative_designs WHERE user_id=? ORDER BY updated_at DESC,id DESC').all(req.user.brave_id)});
});
app.post('/api/creative-designs',requireUser,(req,res)=>{
  const title=clean(req.body.title)||'Untitled BRAVE design';
  const type=clean(req.body.designType)||'social';
  const content=String(req.body.content||'').slice(0,500000);
  if(!content)return res.status(400).json({message:'Design content is required.'});
  const did=id('design');
  db.prepare('INSERT INTO creative_designs(public_id,user_id,title,design_type,content) VALUES(?,?,?,?,?)').run(did,req.user.brave_id,title,type,content);
  res.status(201).json({message:'Design saved to your Creative Studio.',design:db.prepare('SELECT * FROM creative_designs WHERE public_id=?').get(did)});
});
app.put('/api/creative-designs/:id',requireUser,(req,res)=>{
  const title=clean(req.body.title)||'Untitled BRAVE design', content=String(req.body.content||'').slice(0,500000);
  const row=db.prepare('SELECT * FROM creative_designs WHERE public_id=? AND user_id=?').get(req.params.id,req.user.brave_id);
  if(!row)return res.status(404).json({message:'Design not found.'});
  db.prepare('UPDATE creative_designs SET title=?,content=?,updated_at=? WHERE public_id=? AND user_id=?').run(title,content,now(),req.params.id,req.user.brave_id);
  res.json({message:'Design updated.'});
});
app.delete('/api/creative-designs/:id',requireUser,(req,res)=>{
  db.prepare('DELETE FROM creative_designs WHERE public_id=? AND user_id=?').run(req.params.id,req.user.brave_id);
  res.json({message:'Design deleted.'});
});

app.get('/api/account/business-summary',requireUser,(req,res)=>{
  const uid=req.user.brave_id;
  res.json({
    user:{brave_id:req.user.brave_id,username:req.user.username,fullname:req.user.fullname},
    products:db.prepare('SELECT COUNT(*) c FROM products WHERE owner_id=?').get(uid).c,
    services:db.prepare('SELECT COUNT(*) c FROM services WHERE owner_id=?').get(uid).c,
    customers:db.prepare('SELECT COUNT(*) c FROM customers WHERE user_id=?').get(uid).c,
    records:db.prepare('SELECT COUNT(*) c FROM records WHERE user_id=?').get(uid).c,
    receipts:db.prepare('SELECT COUNT(*) c FROM receipts WHERE user_id=?').get(uid).c,
    designs:db.prepare('SELECT COUNT(*) c FROM creative_designs WHERE user_id=?').get(uid).c,
    saved:db.prepare('SELECT COUNT(*) c FROM saved_listings WHERE user_id=?').get(uid).c,
    messages:db.prepare('SELECT COUNT(*) c FROM messages WHERE sender_id=? OR receiver_id=?').get(uid,uid).c
  });
});

app.get('/api/account/reputation',requireUser,(req,res)=>{
  const uid=req.user.brave_id;
  const products=Number(db.prepare('SELECT COUNT(*) c FROM products WHERE owner_id=?').get(uid).c);
  const services=Number(db.prepare('SELECT COUNT(*) c FROM services WHERE owner_id=?').get(uid).c);
  const posts=Number(db.prepare('SELECT COUNT(*) c FROM timeline_posts WHERE user_id=?').get(uid).c);
  const reviews=Number(db.prepare('SELECT COUNT(*) c FROM profile_reviews WHERE profile_user_id=?').get(uid).c || 0);
  const rating=db.prepare('SELECT AVG(rating) avg FROM profile_reviews WHERE profile_user_id=?').get(uid);
  res.json({products,services,posts,reviews,rating:Number(rating?.avg||0)});
});

app.get('/api/admin/chats/overview',requireAdmin,(req,res)=>{
  const messages=db.prepare(`
    SELECT m.*, su.fullname AS sender_name,su.username AS sender_username,
           ru.fullname AS receiver_name,ru.username AS receiver_username
    FROM messages m
    LEFT JOIN users su ON su.brave_id=m.sender_id
    LEFT JOIN users ru ON ru.brave_id=m.receiver_id
    ORDER BY m.id DESC LIMIT 500`).all();
  const ai=db.prepare(`
    SELECT a.*,u.fullname,u.username,u.email
    FROM ai_conversations a LEFT JOIN users u ON u.brave_id=a.user_id
    ORDER BY a.id DESC LIMIT 500`).all();
  res.json({messages,ai});
});
app.post('/api/admin/chats/reply/:userId',requireAdmin,(req,res)=>{
  const userId=clean(req.params.userId), message=clean(req.body.message);
  if(!message)return res.status(400).json({message:'Reply message is required.'});
  const u=db.prepare('SELECT * FROM users WHERE brave_id=?').get(userId);
  if(!u)return res.status(404).json({message:'User not found.'});
  db.prepare('INSERT INTO messages(public_id,sender_id,receiver_id,message) VALUES(?,?,?,?)').run(id('adminmsg'),'UNIQUE_BRAVE_ADMIN',u.brave_id,message);
  db.prepare('INSERT INTO notifications(public_id,user_id,title,message) VALUES(?,?,?,?)').run(id('note'),u.brave_id,'UNIQUE BRAVE Admin message','You received a message from UNIQUE BRAVE administration.');
  audit('admin_chat_reply','user',u.brave_id,message);
  res.status(201).json({message:'Admin message sent.'});
});

app.get('/api/admin/system/health',requireAdmin,(req,res)=>{
  const checks=[];
  try{db.prepare('SELECT 1').get();checks.push({name:'Database connection',ok:true});}catch(e){checks.push({name:'Database connection',ok:false,error:e.message});}
  try{db.prepare('PRAGMA integrity_check').get();checks.push({name:'SQLite integrity check',ok:true});}catch(e){checks.push({name:'SQLite integrity check',ok:false,error:e.message});}
  try{db.prepare('SELECT COUNT(*) c FROM users').get();checks.push({name:'Users table',ok:true});}catch(e){checks.push({name:'Users table',ok:false,error:e.message});}
  try{db.prepare('SELECT COUNT(*) c FROM products').get();checks.push({name:'Products table',ok:true});}catch(e){checks.push({name:'Products table',ok:false,error:e.message});}
  try{db.prepare('SELECT COUNT(*) c FROM messages').get();checks.push({name:'Messages table',ok:true});}catch(e){checks.push({name:'Messages table',ok:false,error:e.message});}
  res.json({node:process.version,platform:process.platform,dbPath:db.name,checks,sessionCount:userSessions.size,adminSessionCount:adminSessions.size});
});
app.post('/api/admin/system/repair',requireAdmin,(req,res)=>{
  const action=clean(req.body.action);
  try{
    if(action==='integrity'){db.prepare('PRAGMA optimize').run?.(); db.prepare('PRAGMA wal_checkpoint(PASSIVE)').run?.(); audit('system_repair','database','integrity'); return res.json({message:'Database maintenance completed.'});}
    if(action==='clear-expired-sessions'){db.prepare('DELETE FROM user_sessions WHERE expires_at<=?').run(now()); audit('system_repair','sessions','expired'); return res.json({message:'Expired sessions cleared.'});}
    if(action==='rebuild-indexes'){db.exec('REINDEX'); audit('system_repair','database','reindex'); return res.json({message:'Database indexes rebuilt.'});}
    return res.status(400).json({message:'Unknown repair action.'});
  }catch(e){return res.status(500).json({message:'Repair failed.',error:e.message});}
});
app.get('/api/admin/system/migrations',requireAdmin,(req,res)=>{
  res.json({migrations:db.prepare('SELECT * FROM brave_migrations ORDER BY name').all()});
});
app.post('/api/admin/system/backup',requireAdmin,(req,res)=>{
  try{
    ...
    fs.mkdirSync(dir,{recursive:true});
    const stamp=new Date().toISOString().replace(/[:.]/g,'-');
    const target=path.join(dir,`manual-backup-${stamp}.db`);
    db.exec('PRAGMA wal_checkpoint(TRUNCATE)');
    fs.copyFileSync(db.name,target);
    audit('manual_database_backup','database',path.basename(target));
    res.json({message:'Database backup created.',file:path.relative(__dirname,target)});
  }catch(e){res.status(500).json({message:'Backup failed.',error:e.message});}
});
