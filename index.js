...
function transporter(){if(!process.env.SMTP_HOST||!process.env.SMTP_USER||!process.env.SMTP_PASS)return null;return nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT||587),secure:Number(process.env.SMTP_PORT||587)===465,auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS}});}
async function sendMail(to,subject,html){const t=transporter();if(!t)return {sent:false};try{await t.sendMail({from:process.env.SMTP_FROM||process.env.SMTP_USER,to,subject,html});return {sent:true};}catch(e){return {sent:false,error:e.message};}}
...
app.post('/forgot-password',async(req,res)=>{
 ...
 res.status(503).json({message:'Password reset email delivery is not configured yet. Add SMTP settings in Render Environment Variables.'});
});
...
app.listen(PORT,()=>console.log(`UNIQUE BRAVE COM backend is running on port ${PORT}`));
